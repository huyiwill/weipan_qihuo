    ...
    <div class="container-fluid">
        <div id="pad-wrapper" class="form-page">
            <div class="row-fluid form-wrapper">
                <!-- left column -->
                <form action="<?php echo U('News/newsedit');?>" method="post" enctype="multipart/form-data">
                <input type="hidden" name="aid" value="<?php echo ($editnew['nid']); ?>"/>
                <div class="span8 column">
					<div class="field-box">
						<label>文章标题：</label>
						<input class="span8" type="text" data-toggle="tooltip" data-trigger="focus" title="请输入文章标题" data-placement="right" name="ntitle" value="<?php echo ($editnew["ntitle"]); ?>"/>
					</div>
					<div class="field-box">
						<label>文章栏目:</label>
						<div class="ui-select span5">
							<select style="width: 100%;" name="ncategory">
									<?php if(is_array($nclist)): $i = 0; $__LIST__ = $nclist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nl): $mod = ($i % 2 );++$i;?><option value="<?php echo ($nl['fid']); ?>" <?php if($nl['fid'] == $editnew['fid']): ?>selected=""<?php endif; ?>><?php echo ($nl['fclass']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
							</select>
						</div>
					</div>
					<div class="field-box">
						<label>缩略图:</label>
						<input class="span6 inline-input" type="text" name="ncover" value="<?php echo ($editnew["ncover"]); ?>" />
					...
					<div class="field-box">
                        <label>更新时间:</label>
                        <input type="text" value="<?php echo (date('m/d/Y',$editnew['ntime'])); ?>" class="input-large datepicker" name="ntime"/>
                    </div>
					<div class="field-box">
						<label>文章内容:</label>
						<div class="wysi-column">
							<textarea id="wysi" class="span10 wysihtml5" rows="20" name="ncontent"><?php echo ($editnew["ncontent"]); ?></textarea>
						</div>
					</div>
