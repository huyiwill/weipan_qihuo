...
(function(e, g) {
	...
	function B(a, b, c) {
		var d = a.toUpperCase().indexOf(b.toUpperCase()),
			b = b.length;		
		var ovalue = this;
		0 > d ? c.push(a) : (c.push(a.substring(0, d)), c.push("<span class='select2-match' value='"+b+"'>"), c.push(a.substring(d, d + b)), c.push("</span>"), c.push(a.substring(d + b, a.length)))
	}
	function C(a) {
		var b, c = 0,
			d = null,
	...
	if (window.Select2 === g) {
		...
		z = x(w, {
			...
			addSelectedChoice: function(a) {
				var b = e("<li class='select2-search-choice'>    <div><input type='checkbox' name='uid' value='' style='display:none;'/></div>    <a href='#' onclick='return false;' class='select2-search-choice-close' tabindex='-1'></a></li>"),
					c = this.id(a),
					d = this.getVal(),
					f;
				...
				//console.log(this.select.context.childNodes[3].value);
				
				b.find("div").replaceWith("<div><input type='checkbox' name='uid' value='"+this.id(a)+"' selected='selected' style='display:none;'/>" + this.opts.escapeMarkup(f) + "</div>");
		...
		e.fn.select2.defaults = {
			...
			formatInputTooShort: function(a, b) {
				return "请输入 " + (b - a.length) + " more characters"
			},
			formatSelectionTooBig: function(a) {
				return "你可以选择 " + a + " item" + (1 == a ? "" : "s")
			},
